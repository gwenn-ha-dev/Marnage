...
> Xcode (`App/Marnage/`), qui est la seule vérité. Ce guide documente la
...
2. Product Name : `Marnage` — Interface : SwiftUI, Language : Swift,
   Testing System : None, Storage : None.
3. Emplacement : le dossier `Marnage/App/`. Xcode crée `Marnage/App/Marnage/`.

## 2. Signature (sans compte payant)

Projet → target `Marnage` → **Signing & Capabilities** :
...
## 3. Brancher MarnageKit

1. **File → Add Package Dependencies… → Add Local…** et choisir le dossier
   `Marnage/` (la racine du package, pas `App/`).
2. Ajouter le produit **MarnageKit** à la target `Marnage`.

## 4. Ajouter la target widget

1. **File → New → Target… → macOS → Widget Extension**.
2. Product Name : `MarnageWidget`. **Décocher** « Include Configuration App Intent ».
3. « Activate scheme » : oui.
4. Target `MarnageWidget` → General → Frameworks and Libraries : ajouter
   **MarnageKit**, et même réglage de signature qu'à l'étape 2.

## 5. Mettre en place les sources

Xcode 16+ adosse chaque target à un **dossier synchronisé** : tout fichier
déposé dans `App/Marnage/Marnage/` (resp. `MarnageWidget/`) entre de
...
1. Supprimer (Move to Trash) les fichiers Swift générés par Xcode :
   - dossier `Marnage` : `ContentView.swift` et `MarnageApp.swift` ;
   - dossier `MarnageWidget` : tous les `.swift` générés (garder `Info.plist`
     et l'asset catalog).
2. Copier les sources de ce dépôt dans les dossiers du projet :
   - `App/Marnage/Marnage/` : `Marnage.swift`, `TideCurve.swift`,
     `TideModel.swift` ;
   - `App/Marnage/MarnageWidget/` : `MarnageWidget.swift`.
...
3. Le widget affiche la même courbe et le même choix de ports que l'app : il
   ...
   **Target Membership** → cocher `MarnageWidgetExtension` en plus de
   `Marnage`. `Marnage.swift` reste réservé à l'app. (Xcode inscrit ces
...
4. Embarquer les constantes **sans les dupliquer** : File → **Add Files to
   "Marnage"…** → choisir le dossier `stations/` à la racine du dépôt, avec
...
1. Scheme `Marnage` → **Run** : la fenêtre affiche les marées de la semaine.
